...
module.exports = function(app) {
    ...
    start: function() {
      ...
      app.get("/appstore/", (req, res, next) => {
        findPluginsAndWebapps()
          .then(([plugins, webapps]) => {
            res.render("appstore", {
              plugins: getRenderInfo(plugins, getPlugin, "/plugins/configure/#"),
              webapps: getRenderInfo(webapps, getWebApp, ""),
              anyInstalled: Object.keys(modulesInstalledSinceStartup).length > 0,
  ...
  function getRenderInfo(modules, existing, pathPrefix)
  {
    return modules.map(plugin => {
      ...
      return {
        ...
        installColumn: getInstallColumn(name, version, installedModule, isLink),
        updateColumn: getUpdateColumn(name, version, installedModule, isLink),
        authorColumn: getAuthorColumn(plugin),
        npmColumn: getNpmColumn(plugin),
        nameColumn: getNameColumn(name, installedModule, pathPrefix),
        descriptionColumn: plugin.package.description
      }
    });
  }
  ...
  function getNameColumn(name, installedModule, pathPrefix) {
    return '<b>' + (installedModule ? `<a href="${pathPrefix}${name}">${name}</a>`: `${name}`) + '</b>'
  }
